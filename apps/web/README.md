...
未登录时显示米家登录页。获取二维码、扫码、取消、重新扫码与安全验证属于同一次登录流程。已保存的授权由 backend 恢复，浏览器不保存账号凭据；恢复失败时可以重试或清除授权。账号窗口内支持重新扫码和退出登录。账号授权与扫码尝试分别读取 `account`、`loginAttempt`；重新扫码不卸载工作台或中断原账号的播放，摄像头选择由账号 ID 隔离。
