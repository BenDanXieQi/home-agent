...
├── devices/
│   ├── discovery.ts                # 唯一设备目录、家庭范围与派生索引
│   ├── queries.ts                  # 家庭／房间聚合及设备规格查询
...
| devices    | 设备目录、家庭范围、索引及规格查询                         | `DeviceQueries` 读取 `DeviceDiscovery`，不保存另一份目录                 |
...
子模块通过具体参数、实例或回调接收所需依赖，不互相导入 service 实例。协议错误在业务边界映射；HTTP 路由只校验请求和调用业务，不创建小米客户端或后台任务。共享业务契约位于 `packages/api/src/contracts/`，HTTP 读取上限等已有跨应用基础能力复用 `packages/api/src/http/`。
...
`properties/read-request.ts` 按设备分组，最多三台并发查询规格；`reader.ts` 管理全服务串行属性批次。公共规格元数据按 model／URN 缓存并合并相同在途请求，每位调用者保留自己的取消信号及 30 秒预算；最后一位等待者取消才中止共享传输。展示翻译失败不抹掉已经取得的能力规格。
...
Step 2 计划接入家庭运行时时，须同批替换对应业务状态来源，不保留两份独立更新的目录。Step 3 计划负责自动采集集合、上线补缺、重连补读、latest／availability 及属性版本仲裁，复用现有读取与观察入口，不再创建另一套协议客户端。
