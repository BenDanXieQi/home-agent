...
本文记录当前设备接入的协议依据、来源契约和验证规范。后续家庭运行时与采集行为只在对应计划中实施。
...
- MiLoCo 账号级订阅属性后在状态写入处过滤当前家庭。本项目沿已确定的单家庭采集范围，适配器只为调用方明确提供并经同账号目录校验的设备集合建立属性／在线订阅；service 校验现有家庭选择和目录，Step 3 计划负责自动生成持续采集集合；目录迁移通知的账号级范围归 Step 2 计划。此差异不改变每设备 topic 和解码方式。
...
- Step 1 不提供家庭 actor 或 scope_epoch；现有持久家庭选择约束所有设备访问。实机验证从当前账号完整成功同步的家庭／设备目录中，由操作者明确选择或明确授权代理选择代表设备及属性；以真实 home_id、did、siid/piid 记录在本机报告。验收不修改现有家庭选择，也不自动选择第一个家庭。
...
- 本机验收接收方只记录消息、确认结果和时间，不维护业务 latest、不推导在线状态或缺值，不参与自动化。Step 1 不把“重新读取成功”表述为“家庭状态已恢复”。Step 2 计划交付家庭运行时；Step 3 计划将选中家庭、属性版本和 availability 接到正式入口。
